...
ни се наоѓа проектот и која што ќе го користи нејзиниот python интерпретер.

За да може да се креира виртуелна околина потребно е да се исталира 
``easy_install`` за python да може да ги исталираме најновите верзии на пакети. 
Се исталира со командата::

   sudo apt-get install python-setuptools

Потоа со помош на него се инсталира virtualenv за креирање на 
виртуелни python околина::

   sudo easy_install virtualenv

Најпрвин креираме нова виртуелна околина за нашиот проект::

   virtualenv <project_name>
  
За ова потребно е да бидеме во фолдерот каде што сакаме да 
биде сместена нашата виртуелна околина.
Пред да се направи активација на виртуелната околина потребно е да се направи 
уште едно подесување во ``bin/activate`` фајлот. На кајот од самиот фајл 
потребно е да се внесеат следните линии код::
...
Откако сме веќе во виртуелната околина потребно е да исталираме ``django`` со 
``pip`` се исталира последната офицјална верзија::
...
Како развојна околина користиме Eclipse со исталиран pydev додаток за python.
Наредно што треба да направиме е да направиме import проектот во Eclipse.
На следниов `линк <http://vaboone.wordpress.com/2010/11/22/import-a-django-project-into-eclipse/>`_ 
детално е објаснето како да го внесете вашиот django проект во Eclipse.

Еден django проект може да се состои од една или повеќе апликации. 
...
Нареден чекор е да ги направиме основните подесувања во ``settings`` фајлот на проектот.
...
За овој проект ќе користиме sqlite3 база на податоци.
...
И на интернет прилистувач венсуваме ``127.0.0.1:8000/admin/`` 
При што треба да се појаву admin панелот. ``(ПРИКАЖИ СЛИКА)``


Модели
------

Моделите ни овозможуваат на поедноставен начин да дадеме репрезентација на 
...
Со креиање на класа ``Person`` со сабкласа  ``models.Model`` ние креиравме 
...
Во Вториот дел од класата имаме дефинирано неколку полиња кои што го 
...
`линк <https://docs.djangoproject.com/en/1.5/ref/models/instances/#django.db.models.Model.__unicode__>`_.
...
Да преминеме сега на креирање на моделите во нашата апликација. 
...
Најпрвин потребно е да искреираме нов модул contact  во кој што ќе го сместиме 
овој модел.
...
Од овие полиња ке се задржам само на кратко на ``gender`` кој што преставува 
...
Како што претходно рековме потребни ни се две релации во овој модел со два 
...
за истот. Што значи ни требаат два дополнителни модели :
...
За да овозможиме коментари на активноста потребно е да се искреира дополнителен 
...
---------------------------------
Откако ги искреиравме моделите и веќе ги имаме сите потребни табели во база, 
...
``database-abstraction API``. Ова API ви ги овозможува сите претходно спомнати 
...
Со ова веќе имаме искреирано нов објект од моделот ``Person`` и истиот е зачуван 
...
Со оваа промена во база се промени само ``first_name`` полето и сега овој 
``person`` е со име New.
...
прикажеме сите person објекти  со име ``Alex`` тоа би изгледало вака::

      alex_persons = Person.objects.filter(first_name='Alex'))
      
Доколку пак сакаме да ги изоставиме сите што имаат презиме ``Deanovski`` го 
пишуваме следново пребарување::
...
поопширно за ова може да најдете `овде <https://docs.djangoproject.com/en/1.5/topics/db/sql/>`_.
  
Креирање на  методи во SocialNet
--------------------------------
За да продолжиме со оваа целина потребно е претходната задача за креирање на 
...
Секој корисник може да си креира свој профил и притоа истиот да го едитира, 
...
- Метод за деактивирање на профилот
...
Методот за креирање на нов корисник односно инстанца од моделот 
Person изгледа вака::
...
Дополнителни работи што е потребно да направиме во овој метод се на ``gender`` 
...
Откако овој метод е креиран и веќе креираните тестови се успешни преминуваме на 
...
Со овој метод правиме update на секое поле што корисникот го променил со тоа што 
...
едитираме полето ``gender``. На крај доколку е потребно да вратиме вредност на 
...
тестови за оваа целина доколку се е во ред се префралуваме на наредниот метод.

Наредниот метод се однесува на деактивирање на веќе активен корисник. 
Се што треба да направиме во овој случај е да го промениме полето ``is_active`` 
од `True` во `False`::
...
Проверуваме тестови и продолжуваме понатаму.


Откако сите методи за овој модел ни се готови преминуваме со креирање на 
...
Потребно е најпрвин да провериме дали овие два корисници веќе не се поврзани 
...
Како што спомнав претходно како задача потребно е да бидат искреирани останатите 
...
Како што спомнавме веќе многу важен дел денес од развивањето на една WEB 
...
За извржување на тестовите во django од командна линија ја извршуваме следната команда::

   django-admin.py test
   
При што се извршуваат сите тестови во проектот доколку сакаме да истестираме 
...
моулот ги чуваме во модул со име ``contact`` итн. За да бидат пронајдени овие 
тестови од страна на django името на тест класата за специфичниот модул мора да 
бидат импортувани во ``init`` модулот на ``tests`` пакетот.
Сите преостанати детали околу ``django unit tests`` може да ги прочитате во 
неговата `документација <https://docs.djangoproject.com/en/1.5/topics/testing/overview/#module-django.test>`_
...
Доколку го имате прочитано веќе погоре дадениот линк со детали околу django 
тестовите јасно ви е дека првиот дел од кодов претставува дефинирање на самата тест класа.
``Fixtures`` претставуваат податоци во ``json`` формат со кои што може да 
...
во ``setUp()`` методот тој се извршува за секој тест посебно.
...
``test`` python односно django го препознаваат како тест. Во нашиов случај 
сакаме да провериме дали со дадените податоци ќе се искреира нов објект од 
моделот Person и дали истиот е снимен во база. Бидејќи веќе во ``fixtures`` 
имаме еден објект од моделот Person кој што е со ``id=1`` наредниот реално е да 
очекуваме да биде со ``id=2``. На истиот начин ги креираме тестовите и за сите 
...
`Попширно <https://docs.djangoproject.com/en/1.5/topics/forms/>`_ за истата може 
...
потребни параметрите ``first_name``, ``last_name`` кои што треба да бидат текст 
...
Forms во SocialNet
------------------
Во продолжение ќе дадеме пример на неколку форми од SocialNet додека остантите 
...
како на пример за ``gender`` потребно е како податоци да примаме само 
...
Последна форма која што ќе ја презентираме овдека е формата за промена на лозинка.::
...
Врз основа на документацијата за ``Django Forms``  и примерите дадени овдека, 
